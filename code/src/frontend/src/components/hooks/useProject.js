...
function useProject({ id }) {
  ...
  const loadProject = async () => {
    ...
    const result = await client.apis.Projects["get_projects__project_id_"]({
      project_id: id,
    });
    setProjectDocument({ ...result.obj, id: id });
    document.title = `${result.obj.name} - Project`;
    try {
      rdflib.parse(
        result.obj.rdf_content,
        store,
        "http://jenplane.org/",
        "text/turtle"
      );
    ...
    const rdfLayoutDATA = [
      ...
      ...activities.map((a, i) => {
        ...
        return {
          ...
          preferredColor: preferredColor.map((c) => c.object.value).join(", "),
  ...
  const saveProject = () => {
    ...
    getOpenAPI()
      .then((client) => {
        return client.apis.Projects["put_projects__project_id_"]({
          project_id: id,
          body: {
            rdf_content: serializedStore,
          },
        });
      })
      .then((result) => {
